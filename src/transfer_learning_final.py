...
def define_model(init_learning_rate):
    """
...
def evaluate(model, X_train, X_test, y_train, y_test, labels, batch_size):
    """
...
This function initialises the argparse and defines the command line parameters 
    ...
    ap.add_argument("-lr","--init_learning_rate",required=True, help = "Desired initial learning rate, e.g. 0.01")
    ap.add_argument("-bs","--batch_size",required=True, help = "Desired batchsize, e.g. 128")
...
The main function defines which functions to run, when the script is run from the terminal. Further this main function includes the training of the model on the training data, and the evaluation of the model.  
    ...
    model = define_model(float(args["init_learning_rate"]))
    
    report = evaluate(model, X_train, X_test, y_train, y_test, labels, int(args["batch_size"]))
